...
def run_all_experiments_pipeline(df, alpha=1.0, beta=1.0, lam=1.0,
    genres=("Minuet", "Allegro", "Adagio"), verbose=True,
    ...
    INPUT:
        ...
        alpha, beta, lam (float): hyperparameters of the Bregman metric.
    ...
    OUTPUT:
        results (dict) with keys:
            ...
            'df_valid' : dataframe with valid original and Bregman features
    ...
    for idx, row in df_run.iterrows():
        ...
        if vec_o is not None:
            ...
            df_run.at[idx, 'features_breg'] = vec_b

    df_valid = df_run.dropna(subset=['features_orig', 'features_breg']).copy()
    ...
    if run_exp1:
        ...
        results['exp1'] = experiment_1(df_valid, genres=genres)

    if run_exp2:
        if verbose:
            print("\n===== RUNNING EXPERIMENT 2 =====")
        results['exp2'] = experiment_2(df_valid)

    if run_exp3a:
        if verbose:
            print("\n===== RUNNING EXPERIMENT 3A =====")
        results['exp3a'] = experiment_3a(df_valid)

    if run_exp3b:
        if verbose:
            print("\n===== RUNNING EXPERIMENT 3B =====")
        results['exp3b'] = experiment_3b(df_valid)

    if run_exp4:
        if verbose:
            print("\n===== RUNNING EXPERIMENT 4 =====")
        results['exp4'] = experiment_4(df_valid)
...
def run_all_experiments_pipeline_pure_js(
    ...
    print("\n===== RUNNING EXPERIMENT 1 =====")
    exp1 = experiment_1(df_valid)

    print("\n===== RUNNING EXPERIMENT 2 =====")
    exp2 = experiment_2(df_valid)

    print("\n===== RUNNING EXPERIMENT 3A =====")
    exp3a = experiment_3a(df_valid)

    print("\n===== RUNNING EXPERIMENT 3B =====")
    exp3b = experiment_3b(df_valid)

    print("\n===== RUNNING EXPERIMENT 4 =====")
    exp4 = experiment_4(df_valid)
