...
def compute_features_for_row(row, alpha=0.75, beta=1.0, lam=1.0, min_num_parts=4):
    ...
    INPUT: row (pandas.Series): a row from a DataFrame. Must contain a key 'parts' that holds a list
        ...
        alpha (float): exponent controlling the rarity penalty in the weighted JS edge-length rule.
    RETURN: tuple of two numpy.ndarrays or (None, None):
            ...
            vec_mod: feature vector for the weighted JS metric.
    ...
    for part in parts[:4]:
        ...
        # Bregman
        dist_dict, mass_dict = assign_distributions(P, eps=1e-8)
        edges_mod = compute_weighted_js_edge_lengths(vertices, vertex_map, P, dist_dict, mass_dict, alpha=alpha,
                                                beta=beta, lam=lam, eps=1e-10, min_weight=1e-8, loop_multiplier=1.0)
        D_mod = compute_original_distance_matrix(vertices, edges_mod)

        finite_vals = D_mod[np.isfinite(D_mod)]
        D_mod[~np.isfinite(D_mod)] = 1.1 * (np.max(finite_vals) if len(finite_vals) > 0 else 1.0)
        dgms_mod = compute_persistence(D_mod)

        for dim in [0, 1]:
            vec_orig.extend(extract_barcode_stats(dgms_orig[dim]))
            vec_mod.extend(extract_barcode_stats(dgms_mod[dim]))

    return np.array(vec_orig, dtype=float), np.array(vec_mod, dtype=float)
...
def compute_features_for_row_js(row, js_scale=1.0, edge_eps=1e-6, min_num_parts=4):
    ...
    RETURN: tuple of two numpy.ndarrays or (None, None):
            vec_orig (numpy.ndarray): 24-dimensional vector for the original distance method.
            vec_mod (numpy.ndarray): 24-dimensional vector for the Bregman distance method.
            If the piece has fewer than 4 parts, both return values are None.
    """
    parts = row.get('parts')
    if not parts or len(parts) < min_num_parts:
    ...
    for part in parts[:4]:
        ...
        # Bregman
        dist_dict, mass_dict = assign_distributions(P, eps=1e-8)
        edges_mod = compute_pure_js_edge_lengths(
